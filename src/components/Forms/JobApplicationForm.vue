<template>
  ...
  <div v-else class="container mx-auto px-6 md:px-96">
    ...
    <div id="addresses">
      ...
      <div class="md:flex justify-between items-center gap-4 mb-12 md:mb-0">
        <div v-for="(placeholder, index) in sections.addresses?.placeholders?.home" :key="index"
             class="mb-4 md:w-1/3">
      ...
      <div class="md:flex justify-between items-center gap-4">
        <div v-for="(placeholder, index) in sections.addresses?.placeholders?.work" :key="index"
             class="mb-4 md:w-1/3">
          <custom-input :placeholder="placeholder"
...
export default defineComponent({
  ...
  methods: {
    async submitForm() {
      ...
      // this.isLoading = true;
      await emailjs.send(process.env.VUE_APP_EMAIL_SERVICE, process.env.VUE_APP_JOB_APPLICATION_EMAIL_TEMPLATE, {
